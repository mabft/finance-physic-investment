...
def compute_hurst(series, max_lag=None):
    ...
    if series[0] > 0 and np.all(series > 0):
        # Looks like prices → convert to log-returns
        series = np.diff(np.log(series))
        n = len(series)
        if max_lag > n // 2:
            max_lag = n // 2
    
    # Generate lag array (log-spaced for better coverage)
...
def compute_all_metrics(prices, lookback_days=60, bins=10):
    ...
    Hurst = compute_hurst(prices)  # Use full series for Hurst (needs more data)
...
def classify_market_state(metrics_dict):
    ...
    # High temperature with direction
    if T > 0.08:
        if M > 0:
    ...
    # Normal temperature — check Hurst regime
...
def screen_score(metrics_dict):
    ...
    # Hurst (25 pts)
    if Hurst > 0.55: score += 25  # Trending — momentum works
    elif Hurst > 0.45: score += 10  # Random
    elif Hurst < 0.45 and M < 0: score += 25  # Mean-reverting + oversold
...
# ============================================================
# Formatting helpers for cron job output
# ============================================================
